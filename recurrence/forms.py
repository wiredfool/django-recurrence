...
class RecurrenceWidget(forms.Textarea):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return safestring.mark_safe(u'%s\n%s' % (
            super(RecurrenceWidget, self).render(name, value, attrs, renderer),
            widget_init_js))
...
def find_recurrence_i18n_js_catalog():
    ...
    try:
        return urls.reverse('javascript-catalog')
    except urls.NoReverseMatch:
        try:
            return urls.reverse(_javascript_catalog, kwargs={'packages': 'recurrence'})
        except urls.NoReverseMatch:
            pass
